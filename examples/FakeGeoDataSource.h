...
public:
    ...
    QString tooltip() const override {
        ...
        if (m_status == "active") info += "\n✅ Active";
        else if (m_status == "maintenance") info += "\n⚠️ Maintenance";
...
private:
    ...
    void createCountry(const QString &id, const QString &name)
    {
        ...
        QStringList statuses = QStringList() << "active" << "maintenance" << "offline";
        ...
        for (auto it = cities.begin(); it != cities.end(); ++it) {
            ...
            for (int i = 0; i < it.value().size(); ++i) {
                ...
                m_data[placeId] = std::make_shared<FakeGeoNodeData>(
                    placeId, "📍 " + it.value()[i], status, true
                );
