...
public:
    ...
    QString tooltip() const override {
        QString size;
        if (m_status == "locked") size = " (locked by system)";
        else if (m_status == "readonly") size = " (read-only)";
        else if (m_status == "hidden") size = " (hidden)";
        else size = " (normal)";
        return QString("📄 %1\nPath: %2\nStatus: %3%4")
            .arg(m_name, m_id, m_status, size);
    }
...
private:
    ...
    void createDrive(const QString &id, const QString &name)
    {
        ...
        QStringList statuses = {"normal", "readonly", "hidden", "locked"};
            ...
            for (const auto &sub : it.value()) {
                ...
                if (!sub.second.isEmpty()) {
                    ...
                    for (int i = 0; i < fileNames.size(); ++i) {
                        ...
                        m_data[fileId] = std::make_shared<FakeFileNodeData>(
                            fileId, "📄 " + fileNames[i], status, false
                        );
                        files.append(fileId);
                    }
                }
